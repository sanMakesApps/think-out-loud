    ...
    <div className="min-h-screen bg-zinc-100 text-zinc-800 flex justify-center md:py-5 px-0 sm:px-4 xs:py-0  overflow-hidden">
      <div className="bg-white lg:h-full shadow-xl rounded-none sm:rounded-2xl p-4 sm:p-8 w-full max-w-[900px] xs:min-h-screen space-y-6">
          ...
          <div className="flex justify-center space-x-4">
            {!isListening ? (
              <button
                ...
                className="flex items-center gap-x-2 bg-black text-white px-6 py-2 rounded-full shadow cursor-pointer hover:bg-zinc-700 transition"
                aria-label="Start listening"
              >
                <img src={micWhiteImage} alt="mic" width={18} height={22} />
                <span className="hidden sm:inline">Start Streaming</span>
              </button>
            ) : (
              <button
                onClick={stopListening}
                className="flex items-center gap-x-2 bg-red-500 text-white px-6 py-2 rounded-full shadow hover:bg-red-600 transition"
        ...
        <div className="flex-1 h-full space-y-4 ">
          ...
          {!error && report && <ReportItem report={report} />}
        </div>
      </div>
    </div>
  );
